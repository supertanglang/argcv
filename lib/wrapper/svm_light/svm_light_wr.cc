 ...
 public:
  ...
  bool load_docs(dataset<double, double> data) {
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      _docs[i] =
          create_example(i, 0, 0, 1.0, create_svector(_words, comments, 1.0));
...
svm_light_wr::svm_light_wr() : _sl_info(nullptr) {
  init(dataset<double, double>());
}

svm_light_wr::svm_light_wr(dataset<double, double> d) : _sl_info(nullptr) {
  init(d);
}
...
bool svm_light_wr::learn() {
  ...
  KERNEL_CACHE *_kernel_cache =
      _sl_info->kernel_parm.kernel_type == LINEAR
          ? nullptr
          : kernel_cache_init(_sl_info->totdoc,
                              _sl_info->learn_parm.kernel_cache_size);
  ...
  KERNEL_CACHE **__kernel_cache =
      _kernel_cache == nullptr ? nullptr : &_kernel_cache;
  ...
  if (_sl_info->learn_parm.type == CLASSIFICATION) {
    svm_learn_classification(
        _sl_info->_docs, _sl_info->_target, _sl_info->totdoc,
        _sl_info->totwords, &(_sl_info->learn_parm), &(_sl_info->kernel_parm),
        _kernel_cache, _sl_info->_model, _sl_info->_alpha_in);
  } else if (_sl_info->learn_parm.type == REGRESSION) {
    svm_learn_regression(_sl_info->_docs, _sl_info->_target, _sl_info->totdoc,
                         _sl_info->totwords, &(_sl_info->learn_parm),
                         &(_sl_info->kernel_parm), __kernel_cache,
                         _sl_info->_model);
  } else if (_sl_info->learn_parm.type == RANKING) {
    svm_learn_ranking(_sl_info->_docs, _sl_info->_target, _sl_info->totdoc,
                      _sl_info->totwords, &(_sl_info->learn_parm),
                      &(_sl_info->kernel_parm), __kernel_cache,
                      _sl_info->_model);
  } else if (_sl_info->learn_parm.type == OPTIMIZATION) {
    svm_learn_optimization(_sl_info->_docs, _sl_info->_target, _sl_info->totdoc,
                           _sl_info->totwords, &(_sl_info->learn_parm),
                           &(_sl_info->kernel_parm), _kernel_cache,
...
double svm_light_wr::predict(std::vector<double> x) {
  ...
  if (_sl_info->_model->kernel_parm.kernel_type == LINEAR) { /* linear kernel */
    for (int j = 0; (_words[j]).wnum != 0; j++) { /* Check if feature numbers */
      if ((_words[j]).wnum >
          _sl_info->_model->totwords) /* are not larger than in     */
...
bool svm_light_wr::opt(const std::string &key, const std::string &value) {
  return _sl_info->opt(key, value);
}
bool svm_light_wr::opt(const std::string &key, uint64_t value) {
  return _sl_info->opt(key, value);
}
bool svm_light_wr::opt(const std::string &key, double value) {
  return _sl_info->opt(key, value);
}
bool svm_light_wr::opt(const std::string &key, bool value) {
  return _sl_info->opt(key, value);
}
